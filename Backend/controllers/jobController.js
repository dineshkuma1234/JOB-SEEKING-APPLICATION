...
export const getAllJobs=catchAsyncError(async(req,res,next)=>{
    const jobs=await Job.find({expired:false}).sort({createdAt: -1});
    res.status(200).json({
        success: true,
         jobs,
    });
...
export const postJob=catchAsyncError(async (req,res,next) => {
    const {role}=req.user;
    if (role === "Job Seeker"){
        return next(new ErrorHandler("Job Seeker is not allowed to acess this resources!", 403));
    }
...
export const getMyJobs=catchAsyncError(async(req,res,next)=>{
    ...
    const myJobs=await Job.find({postedBy:req.user._id}).sort({createdAt: -1});
    res.status(200).json({
        success: true,
         myJobs,
    });
